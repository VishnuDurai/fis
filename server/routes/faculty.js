...
router.get('/appraisal/template', authenticateToken, (req, res) => {
  db.all('SELECT * FROM appraisal_template ORDER BY display_order ASC, id ASC', [], (err, rows) => {
    ...
    const defaultFpiItems = [
      ...
      { section_code: 'PART_C', section_title: 'PART C: Research & Consultancy', criteria_code: 'C4', criteria_title: 'IPR & Patents (Filed / Published / Granted)', rubric_description: 'Automatic mapping: 10 marks for patent granted, 7 marks for published, 3 marks for filed [Max 10 pts].', mapping_type: 'auto', fixed_mark_per_record: 10, max_marks: 10, calculation_rule: 'patent_status_split', bracket_config: JSON.stringify({ granted_score: 10, published_score: 7, filed_score: 3 }), display_order: 18 },
      { section_code: 'PART_C', section_title: 'PART C: Research & Consultancy', criteria_code: 'C5', criteria_title: 'Research Grants & External Sponsored Projects', rubric_description: 'Automatic mapping: 10 marks for sanctioned grant >5 Lakhs, 8 marks for <=5 Lakhs, 5 per proposal [Max 15 pts].', mapping_type: 'auto', fixed_mark_per_record: 10, max_marks: 15, calculation_rule: 'bracket_rating', bracket_config: JSON.stringify({ high_grant_score: 10, low_grant_score: 8, proposal_score: 5 }), display_order: 19 },
      ...
      { section_code: 'PART_C', section_title: 'PART C: Research & Consultancy', criteria_code: 'C7', criteria_title: 'Research Scholars Guidance (Ph.D)', rubric_description: 'Automatic mapping: 2.5 marks per registered Ph.D scholar (N/A for Non-Supervisors) [Max 5 pts].', mapping_type: 'auto', fixed_mark_per_record: 2.5, max_marks: 5, calculation_rule: 'phd_supervisor_gated', bracket_config: JSON.stringify({ scholar_unit_score: 2.5 }), display_order: 21 },
      ...
      { section_code: 'PART_D', section_title: 'PART D: Institutional Development & Contribution', criteria_code: 'D1', criteria_title: 'Assigned Institutional & Departmental Responsibilities', rubric_description: 'Automatic mapping: 10 marks per Institutional role (Max 20), 10 per Departmental role (Max 10). Combined Max 20.', mapping_type: 'auto', fixed_mark_per_record: 10, max_marks: 20, calculation_rule: 'fixed_per_record', bracket_config: null, display_order: 23 },
...
router.post('/appraisal/template', authenticateToken, async (req, res) => {
  ...
  try {
      ...
      await new Promise((resolve, reject) => {
        db.run(`
          INSERT INTO appraisal_template (
            ...
            calculation_rule, bracket_config, display_order
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `, [
    ...
    return res.json({ success: true, message: 'Appraisal template, rubrics, fixed marks, max marks, and calculation rules saved successfully!' });
  } catch (err) {
