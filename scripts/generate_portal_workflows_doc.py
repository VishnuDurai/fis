...
def create_portal_workflows_document():
    ...
    fac_workflows = [
        ...
        ("Workflow 1.8 — 1-Click AI CV & Bio-Data Generator: ", "Faculty opens /cv-generator to generate comprehensive, professional resumes. Selects from 3 standard formats (SREC Official Letterhead, AICTE/Anna University Statutory Format, Modern Technical Europass). Chooses AI Summary style (Executive, Research, Teaching), customizes section visibility toggles, and exports high-definition vector PDFs or prints directly."),
        ("Workflow 1.9 — Real-Time Web Push Notification Enrollment: ", "Faculty clicks Notification Bell in Navbar to enable browser/mobile push alerts via Service Worker. Receives instant real-time alerts on appraisal status approvals, bulk sign-offs, revision requests, and administrative circulars with zero SMS latency."),
        ("Workflow 1.10 — Profile Picture Upload & Automated White Background Standardization: ", "Faculty accesses Settings & Security (/settings) to upload a new profile picture (JPG/PNG). The backend automatically processes the image via AI portrait segmentation (rembg) to isolate the portrait and standardize the background to pure solid white (#FFFFFF), automatically updating the avatar across Navbar, Dashboard, and CV.")
    ]
    ...
    hod_workflows = [
        ("Workflow 2.1 — Department Faculty Directory & Read-Only Profile Verification: ", "HOD/Dept Admin accesses Faculty Directory (/admin/faculty) to view department faculty listings with access to 'View Dossier' and 'Docs' ZIP downloads. HR service certificates (Experience, Salary, Relieving) are restricted exclusively to System/Executive Admins. Opens Personal Details (/profile/personal) to inspect DOB, Gender, Mobile, PAN, Aadhaar, and Address. All input fields are explicitly disabled (disabled={auth.role === 'dept_admin'}) to prevent unauthorized editing."),
