...
header("Content-Disposition: attachment; filename=export_innovative.xls");
...
$query = mysqli_query($conn,"select a.Department,i.staff_id,i.staff_name,i.project_title,i.description,i.status from staff_academics a,staff_innovative i where i.staff_id=a.staff_id and a.Department='".$dept."'");
...
if($rowcount > 0){
    while($row = mysqli_fetch_assoc($query)){
        $post_list[] = array("staff_id"=>$row["staff_id"], "staff_name"=>$row["staff_name"], "project_title"=>$row["project_title"], "description"=>$row["description"], "status"=>$row["status"]);
    }
