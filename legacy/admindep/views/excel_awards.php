...
header("Content-Disposition: attachment; filename=export_awards.xls");
...
$query = mysqli_query($conn,"select a.Department,i.file,i.staff_id,i.staff_name,i.awardname,i.awardby,i.event,i.awa_date from staff_academics a,staff_award i where i.staff_id=a.staff_id and a.Department='".$dept."' order by i.awa_date"); 
...
if($rowcount > 0){
    while($row = mysqli_fetch_assoc($query)){
        $post_list[] = array("staff_id"=>$row["staff_id"], "staff_name"=>$row["staff_name"], "awardname"=>$row["awardname"], "awardby"=>$row["awardby"], "event"=>$row["event"], "awa_date"=>$row["awa_date"]);
    }
