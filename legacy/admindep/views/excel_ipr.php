...
header("Content-Disposition: attachment; filename=export_ipr.xls");
...
$query = mysqli_query($conn,"select a.Department,i.staff_id,i.staff_name,i.patent,i.institution,i.generation,i.propose,i.date from staff_academics a,staff_ipr i where i.staff_id=a.staff_id and a.Department='".$dept."'");
...
if($rowcount > 0){
    while($row = mysqli_fetch_assoc($query)){
        $post_list[] = array("staff_id"=>$row["staff_id"], "staff_name"=>$row["staff_name"], "patent"=>$row["patent"], "institution"=>$row["institution"], "generation"=>$row["generation"], "propose"=>$row["propose"]);
    }
