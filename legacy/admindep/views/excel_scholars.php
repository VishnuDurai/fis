...
$query = mysqli_query($conn,"select a.Department,i.staff_id,i.res_id,i.staff_name,i.university,i.sup_name,i.desgination,i.organisation,i.status,i.file from staff_academics a,staff_scholars i where i.staff_id=a.staff_id and a.Department='".$dept."'");
...
if($rowcount > 0){
    while($row = mysqli_fetch_assoc($query)){
        $post_list[] = array("staff_id"=>$row["staff_id"], "res_id"=>$row["res_id"], "staff_name"=>$row["staff_name"], "university"=>$row["university"], "sup_name"=>$row["sup_name"], "desgination"=>$row["desgination"], "organisation"=>$row["organisation"], "status"=>$row["status"]);
    }
