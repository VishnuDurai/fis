...
<body bgcolor="tan">
  ...
  <center><h3 style="color: #682D87;">Conference Publication Report</h3></center>
     <hr>
<div class="form-inline">
<a href="../views/cpublicationtable.php"><button type="button" style="margin-left: 30px; cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('../views/header.php');?><center>
         <?php
         $sql = mysqli_query($conn,"select distinct a.Department from staff_academics a,staff_publication i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
      ...
      <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
        ...
        ?><br><div class="row">
          ...
          <div class="col">
            <b><span><label>Conference Publication Report</label></span></b>
          </div>
          <div class="col">
        ...
        <?php
        $sql = mysqli_query($conn,"select distinct type from staff_resource where to_date between '".$_POST['from']."' and '".$_POST['to']."'");
      while($row = mysqli_fetch_array($sql))  {
        $depts = $row['type'];?>
...
$sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.title,i.journel,i.date_con,i.organizer from staff_academics a,staff_publication i where a.staff_id = i.staff_id and date_con between '".$_POST['from']."' and '".$_POST['to']."' order by a.Department,a.Designation desc,i.date_con");
$s =1;
while($row = mysqli_fetch_array($sql)){
  $name = $row['staff_name'];
  $des = $row['Department'];
...
$sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.title,i.journel,i.date_con,i.organizer from staff_academics a,staff_publication i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."' and i.date_con between '".$_POST['from']."' and '".$_POST['to']."' order by a.Designation desc,i.date_con");
$s =1;
while($row = mysqli_fetch_array($sql)){
  $name = $row['staff_name'];
  $des = $row['Department'];
  ...
  $s++;
      ...
      $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.title,i.journel,i.date_con,i.organizer from staff_academics a,staff_publication i where a.staff_id = i.staff_id order by a.Department,a.Designation desc,i.date_con");
      $s =1;
      while($row = mysqli_fetch_array($sql)){
        $name = $row['staff_name'];
        $des = $row['Department'];
