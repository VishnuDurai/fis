...
<a href="../views/professionaltable.php"><button type="button" style="cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('../views/header.php');?><center>
         <?php
         $sql = mysqli_query($conn,"select distinct a.Department from staff_academics a,staff_member i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
         while($row = mysqli_fetch_array($sql)) {
           ...
           ?><?php
           $sql = mysqli_query($conn,"select distinct a.Department,i.staff_id from staff_academics a,staff_member i where a.staff_id = i.staff_id and i.staff_id='".$_POST['id']."'");
           while($row = mysqli_fetch_array($sql)) {
             $dep = $row['Department'];
             $id = $row['staff_id'];?>
             <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
             ...
             <br><div class="row">
               ...
               <div class="col">
                 <b><span><label>Professional Details</label></span></b>
               </div>
               <div class="col">
...
<table class="table table-sm table-bordered">
         <thead class="table-sm table-success">
           <tr>
             ...
             <th style="width:100px; text-align:center;">Membership Id</th>
...
$sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_id,i.staff_name,i.membershipid,i.organization from staff_academics a,staff_member i where a.staff_id=i.staff_id order by a.Department,a.Designation desc");
  $s =1;
  while($row = mysqli_fetch_array($sql)){
    $staffname = $row['staff_name'];
    $des=$row['Designation'];
    $dep=$row['Department'];
    $mem = $row['membershipid'];
  ...
  <tbody>
    ...
    <td style="width:200px; text-align:center;"><?php echo $mem;?></td>
   ...
   <?php
    $s++;
        ...
        $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_id,i.staff_name,i.membershipid,i.organization from staff_academics a,staff_member i where a.staff_id=i.staff_id and staff_id='".$_POST['id']."'");
        $s =1;
        while($row = mysqli_fetch_array($sql)){
          $staffname = $row['staff_name'];
          $des=$row['Designation'];
        ...
        $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_id,i.staff_name,i.membershipid,i.organization from staff_academics a,staff_member i where a.staff_id=i.staff_id and i.organization='".$_POST['organization']."' order by a.Department,a.Designation desc");
        $s =1;
        while($row = mysqli_fetch_array($sql)){
          $staffname = $row['staff_name'];
          $des=$row['Designation'];
        ...
        $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_id,i.staff_name,i.membershipid,i.organization from staff_academics a,staff_member i where a.staff_id=i.staff_id and Department='".$_POST['dept']."'");
        $s =1;
        while($row = mysqli_fetch_array($sql)){
          $staffname = $row['staff_name'];
          $des=$row['Designation'];
