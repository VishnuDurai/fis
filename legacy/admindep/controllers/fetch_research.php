...
<body bgcolor="tan">
 ...
 <a href="../views/researchtable.php"><button type="button" style=" cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
 <div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('../views/header.php');?><center>
         ...
         <?php
         $sql = mysqli_query($conn,"select distinct type from staff_development where type='".$_POST['type']."'");
       while($row = mysqli_fetch_array($sql))  {
         $depts = $row['type'];?>
...
$sql = mysqli_query($conn,"select a.Department,a.Designation,i.type,i.staff_name,i.coname,i.title,i.from_date,i.to_date,i.year_aca,i.status,i.institution,i.revenue from staff_academics a,staff_development i where a.staff_id = i.staff_id and Department='".$_POST['dept']."'order by a.Designation desc,i.from_date");
$s =1;
while($row = mysqli_fetch_array($sql)){
  $name = $row['staff_name'];
  $des = $row['Designation'];
  ...
  $s++;
      ...
      $sql = mysqli_query($conn,"select a.Department,a.Designation,i.type,i.staff_name,i.coname,i.title,i.from_date,i.to_date,i.year_aca,i.status,i.institution,i.revenue from staff_academics a,staff_development i where a.staff_id = i.staff_id and i.staff_id='".$_POST['staffid']."' and ((i.from_date between '".$_POST['from']."' and '".$_POST['to']."') or (i.to_date between '".$_POST['from']."' and '".$_POST['to']."') or (i.status='On Going')) order by a.Department,a.Designation desc,i.from_date");
$s =1;
while($row = mysqli_fetch_array($sql)){
  $name = $row['staff_name'];
  $des = $row['Designation'];
  ...
  $sql = mysqli_query($conn,"select a.Department,a.Designation,i.type,i.staff_name,i.coname,i.title,i.from_date,i.to_date,i.year_aca,i.status,i.institution,i.revenue from staff_academics a,staff_development i where a.staff_id = i.staff_id order by a.Department,a.Designation desc,i.from_date");
  $s =1;
  while($row = mysqli_fetch_array($sql)){
    $name = $row['staff_name'];
    $des = $row['Designation'];
