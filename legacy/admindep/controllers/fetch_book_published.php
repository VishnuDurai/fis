...
<a href="../views/bookpublishedtable.php"><button type="button" style="margin-left: 30px; cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php
       $sql = mysqli_query($conn,"select distinct a.Department from staff_academics a,staff_book_published i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
       while($row = mysqli_fetch_array($sql)) {
        ...
        ?>
         ...
         ?>
         <?php
         $sql = mysqli_query($conn,"select distinct a.Department,i.staff_id from staff_academics a,staff_book_published i where a.staff_id = i.staff_id and i.staff_id='".$_POST['id']."'");
         while($row = mysqli_fetch_array($sql)) {
           $dep = $row['Department'];
           $id = $row['staff_id'];?>
           <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
           ...
           <br><div class="row">
             ...
             <div class="col">
               <b><span><label>Book Published Details</label></span></b>
             </div>
             <div class="col">
         ...
         <thead class="table-sm table-success">
           <tr>
             ...
             <th style="width:100px; text-align:center;">DateOfPublication</th>
        ...
        if(isset($_POST['Getid'])){
          $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.title,i.coauthor,i.publisher,i.edition,i.isbn,i.dateofpublication from staff_academics a,staff_book_published i where a.staff_id = i.staff_id and i.staff_id='".$_POST['id']."' order by a.Designation desc");
          $s =1;
          while($row = mysqli_fetch_array($sql)){
            $name = $row['staff_name'];
            $des = $row['Designation'];
          ...
          $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.title,i.coauthor,i.publisher,i.edition,i.isbn,i.dateofpublication from staff_academics a,staff_book_published i where a.staff_id = i.staff_id order by a.Designation desc");
          $s =1;
          while($row = mysqli_fetch_array($sql)){
            $name = $row['staff_name'];
            $des = $row['Designation'];
        ...
        $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.title,i.coauthor,i.publisher,i.edition,i.isbn,i.dateofpublication from staff_academics a,staff_book_published i where a.staff_id=i.staff_id and a.Department='".$_POST['dept']."'");
        $s =1;
          while($row = mysqli_fetch_array($sql)){
            $name = $row['staff_name'];
            $dep = $row['Department'];
