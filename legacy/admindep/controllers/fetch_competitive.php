...
<a href="../views/competitivetable.php"><button type="button" style=" cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('../views/header.php');?><center>
         <?php
         $sql = mysqli_query($conn,"select distinct a.Department from staff_academics a,staff_competitive i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
         while($row = mysqli_fetch_array($sql)) {
           ...
           ?>
           <?php
           $sql = mysqli_query($conn,"select distinct a.Department,i.staff_id from staff_academics a,staff_competitive i where a.staff_id = i.staff_id and i.staff_id='".$_POST['id']."'");
           while($row = mysqli_fetch_array($sql)) {
             $dep = $row['Department'];
             $id = $row['staff_id'];?>
             <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
...
$sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.exam_name,i.level,i.score,i.date_of_certificate from staff_academics a,staff_competitive i order by a.Department,a.Designation desc,i.date_of_certificate");
  $s =1;
  while($row = mysqli_fetch_array($sql)){
    $name = $row['staff_id'];
  ...
  <tbody>
   ...
   <?php
    ...
    $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.exam_name,i.level,i.score,i.date_of_certificate from staff_academics a,staff_competitive i where a.staff_id=i.staff_id order by a.Department,a.Designation desc,i.date_of_certificate");
    $s =1;
    while($row = mysqli_fetch_array($sql)){
      $name = $row['staff_id'];
      $staffname = $row['staff_name'];
    ...
    <tbody>
     ...
     <?php
      ...
      $sql = mysqli_query($conn,"select a.Department,a.Designation,i.staff_name,i.exam_name,i.level,i.score,i.date_of_certificate from staff_academics a,staff_competitive i where a.staff_id=i.staff_id and Department='".$_POST['dept']."' order by a.Designation desc,i.date_of_certificate");
      $s =1;
      while($row = mysqli_fetch_array($sql)){
        $name = $row['staff_id'];
        $staffname = $row['staff_name'];
