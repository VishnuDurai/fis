...
$query = mysqli_query($conn,"SELECT * FROM staff_ipr WHERE staff_id = '$staff_id'");
...
if($rowcount > 0){
    while($row = mysqli_fetch_assoc($query)){
        $post_list[] = array("staff_name"=>$row["staff_name"], "patent"=>$row["patent"], "institution"=>$row["institution"], "generation"=>$row["generation"], "propose"=>$row["propose"]);
    }
