...
$query = mysqli_query($conn,"SELECT * FROM staff_award WHERE staff_id = '$staff_id'");
...
if($rowcount > 0){
    while($row = mysqli_fetch_assoc($query)){
        $post_list[] = array("staff_name"=>$row["staff_name"], "awardname"=>$row["awardname"], "awardby"=>$row["awardby"], "event"=>$row["event"], "awa_date"=>$row["awa_date"]);
    }
