...
$query = mysqli_query($conn,"SELECT * FROM staff_interaction WHERE staff_id = '$staff_id'");
...
if($rowcount > 0){
    while($row = mysqli_fetch_assoc($query)){
        $post_list[] = array("staff_name"=>$row["staff_name"], "type"=>$row["type"], "title"=>$row["title"], "from_date"=>$row["from_date"], "to_date"=>$row["to_date"], "organizer"=>$row["organizer"]);
    }
