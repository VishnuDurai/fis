...
<body bgcolor="tan">
  ...
  <center><h3 style="color: #682D87;">Resource Person</h3></center>
     <hr>
<div class="form-inline">
<a href="resourcetable.php"><button type="button" style="cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('header.php');?><center>
         <?php
         $sql = mysql_query("select distinct a.Department from staff_academics a,staff_resource i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
      ...
      <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
        ...
        ?><br><div class="row">
          ...
          <div class="col">
            <b><span><label>Resource Person Details</label></span></b>
          </div>
          <div class="col">
        ...
        <?php
        $sql = mysql_query("select distinct type from staff_resource where type='".$_POST['type']."' and to_date between '".$_POST['from']."' and '".$_POST['to']."' order by a.Department,a.Designation desc,i.from_date");
      while($row = mysql_fetch_array($sql))  {
        $depts = $row['type'];?>
...
$sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.type,i.title,i.actedas,i.from_date,i.to_date,i.organizer from staff_academics a,staff_resource i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."' order by a.Designation desc,i.from_date");
  $s =1;
  while($row = mysql_fetch_array($sql)){
    $name = $row['staff_name'];
    $des = $row['Designation'];
    ...
    $s++;
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.type,i.title,i.actedas,i.from_date,i.to_date,i.organizer from staff_academics a,staff_resource i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."' and i.type='".$_POST['type']."' and i.to_date between '".$_POST['from']."' and '".$_POST['to']."' order by a.Design desc,i.from_date");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.type,i.title,i.actedas,i.from_date,i.to_date,i.organizer from staff_academics a,staff_resource i where a.staff_id = i.staff_id order by a.Department,a.Designation desc,i.from_date");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
