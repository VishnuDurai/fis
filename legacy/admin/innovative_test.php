...
<body bgcolor="tan"><br>
    <?php
$sql = "select * from staff_innovative order by staff_id";
...
<div style="color: #682D87;" class="news"><b><marquee behavior="alternate">Innovative Project Details</marquee></b></div>
 <center><h3></h3></center>
  <hr>
  <div id="form-control">
...
<a href="innovative_test.php"><button type="button" style="margin-left: 30px; cursor: pointer;"
...
<table class="table table-bordered table-sm table-hover table-striped" id="myTable" style=" margin-top: 40px;">
  ...
  <?php
  $sql = "select a.Department,a.Designation,i.id,i.staff_id,i.staff_name,i.project_title,i.description,i.status,i.date from staff_academics a,staff_innovative i where i.staff_id=a.staff_id";
  ...
  while($abc = mysql_fetch_array($result)){
      ...
      $patent = $abc['project_title'];
      $institution = $abc['description'];
      $generation = $abc['status'];
      $file = $abc['file'];
    ?>
  ...
  <tr>
    ...
    <td><?php echo $abc['project_title']; ?></td>
    <td><?php echo $abc['description']; ?></td>
    <td><?php echo $abc['status']?></td>
    <td><?php echo $abc['date']?></td>
    
    <td><?php echo "<a href='modify_innovative.php?id=$id&staff_id=$staff_id&staff_name=$staff_name&project_title=$patent&description=$institution&status=$generation' >Modify</a>" ?> </td>
  <td><?php echo "<a href='innovative_test.php?del=$abc[id]'>Delete</a>"; ?></td>
  </tr>
  </tbody>
  <?php
  }
...
{
	$id = $_GET['del'];
	$sql = "delete from staff_innovative where id='$id'";
