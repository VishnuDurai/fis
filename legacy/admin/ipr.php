...
<table class="table table-bordered table-sm table-hover table-striped" id="myTable" style=" margin-top: 40px;">
  ...
  <th>Patent generation Date</th>
  <th>Propose of IPR</th>
  <th>Date of Entry</th>
      <th>View</th>
  <th>Update</th>
      <th>Delete</th>
  ...
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:110px; border:none; background-color:#FDF8E4;" value="'.$abc['patent'].'" name="patent'.$i.'" />';
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:150px; border:none; background-color:#FDF8E4;" value="'.$abc['institution'].'" name="institution'.$i.'" />';
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:100px; border:none; background-color:#FDF8E4;" value="'.$abc['generation'].'" name="generation'.$i.'" />';
  echo "</td>";
   echo "<td>";
  echo '<input type="text" style="width:170px; border:none; background-color:#FDF8E4;" value="'.$abc['propose'].'" name="propose'.$i.'" />';
  echo "</td>";
          echo "<td>";
...

  {
    if(isset($_POST['check'.$i.'']))
    {
      $id = $_POST['id'.$i.''];
      $type = $_POST['patent'.$i.''];
      $title = $_POST['institution'.$i.''];
      $from = $_POST['generation'.$i.''];
      $to=$_POST['propose'.$i.''];

      $update = "update staff_ipr set patent='$type',institution='$title',generation='$from',propose='$to' where id='$id' ";
...
echo "<script type='text/javascript'>alert('Updated successfully !')</script>";
  ...
  if(isset($_POST['delete'.$i.'']))
  {
    if(isset($_POST['check'.$i.''])){
      ...
      $delete = "delete from staff_ipr where id='$id'";
