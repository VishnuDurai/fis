...
<div id="page">
	...
	<div style="color: #682D87;" class="news"><b><marquee behavior="alternate">Interaction Details</marquee></b></div>
<form action=" " onsubmit="return registerValidate(this)" enctype="multipart/form-data" method="post"><hr>
<div class="form-control"><br>
			<center><h3 style="color: #682D87;" class="form-group">Fill Your Interaction Details</h3></center><hr>
		<table align="center">
...
<td><input type="text" class="form-control" name="staff_name"
  ...
  ?>"
   style='background-color:white; font-weight:bold;' readonly></td></tr>
<tr><td>Type</td>
  <td><input type="text" class="form-control" name="type" class="form-control"
    value="<?php
    if(isset($_GET['id'])){
      $id = $_GET['id'];
      $type = $_GET['type'];
...
<tr><td>From Date</td>
<td><input type="date" class="form-control" name="from"
  value="<?php
  if(isset($_GET['id'])){
    $id = $_GET['id'];
    $from = $_GET['from_date'];
...
<tr><td>&nbsp;</td><td><br>
  <input type="submit" class="btn btn-outline-success" onclick="window.location.reload(true)" style="cursor: pointer;" name="submit" value="Update"></td><td><br><center>
	<a href="interaction_test.php"><button type="button" style="cursor: pointer;" class="btn btn-outline-primary">Back</button></center></td></tr>
</table><br>
</div>
</form>
<hr>
...
if(isset($_GET['id'])){
  if(isset($_POST['submit'])){
    ...
    $to = $_POST['to'];
    $org = $_POST['organizer'];
    $sql = mysql_query("update staff_interaction set type='$type',title='$title',from_date='$from',to_date='$to',organizer='$org' where id='$id'");
if($sql){?>
	<script>
...
if(isset($_GET['id'])){
	if(isset($_POST['sub_file'])){
	 ...
	 if(move_uploaded_file($file_loc,$folder.$final_file)){
		 $sql = mysql_query("update staff_interaction set file='$final_file' where id = '$id'");
		?>
		<script>
	  alert('successfully uploaded');
	        window.location.href='interaction_test.php?success';
