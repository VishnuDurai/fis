...
<a href="certificatetable.php"><button type="button" style="cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('header.php');?><center>
         <?php
         $sql = mysql_query("select distinct a.Department from staff_academics a,staff_certificate i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
         while($row = mysql_fetch_array($sql)) {
           ...
           ?>
           <?php
           $sql = mysql_query("select distinct a.Department,i.staff_id from staff_academics a,staff_certificate i where a.staff_id = i.staff_id and i.staff_id='".$_POST['staffid']."'");
           while($row = mysql_fetch_array($sql)) {
             $dep = $row['Department'];
             $id = $row['staff_id'];?>
             <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
             ...
             <br><div class="row">
               ...
               <div class="col">
                 <b><span><label>Online Certification Report</label></span></b>
               </div>
               <div class="col">
...
<table class="table table-sm table-bordered">
  ...
  <th style="width:100px; text-align:center;">Date</th>
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.staff_id,i.staff_name,i.course_name,i.mark,i.organisation,i.data_of_exam from staff_academics a,staff_certificate i where a.staff_id=i.staff_id and i.staff_id='".$_POST['staffid']."'");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
          $dep = $row['Department'];
          $type = $row['course_name'];
          $title = $row['mark'];
          $from = $row['organisation'];
          $date = $row['data_of_exam'];
                ?>
        <tbody>
          <td><?php echo $s;?></td>
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.staff_id,i.staff_name,i.course_name,i.mark,i.organisation,i.data_of_exam from staff_academics a,staff_certificate i where a.staff_id=i.staff_id and a.Department='".$_POST['dept']."' order by a.Designation desc,i.data_of_exam");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.staff_id,i.staff_name,i.course_name,i.mark,i.organisation,i.data_of_exam from staff_academics a,staff_certificate i where a.staff_id=i.staff_id order by a.Department,a.Designation desc,i.data_of_exam");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
      ...
      if(isset($_POST['GetDate'])){
        $sql = mysql_query("select a.Department,a.Designation,i.staff_id,i.staff_name,i.course_name,i.mark,i.organisation,i.data_of_exam from staff_academics a,staff_certificate i where a.staff_id=i.staff_id and i.data_of_exam between '".$_POST['from']."' and '".$_POST['to']."' order by a.Department,a.Designation desc,i.data_of_exam");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
