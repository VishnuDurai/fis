...
<body bgcolor="tan">
  ...
  <center><h3 style="color: #682D87;">Innovative Project Details</h3></center>
     <hr>
     <div class="form-inline">
<a href="innovativetable.php"><button type="button" style=" cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('header.php');?><center>
         <?php
         $sql = mysql_query("select distinct a.Department from staff_academics a,staff_innovative i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
         while($row = mysql_fetch_array($sql)) {
           ...
           ?>
           <?php
           $sql = mysql_query("select distinct a.Department,i.staff_id from staff_academics a,staff_innovative i where a.staff_id = i.staff_id and i.staff_id='".$_POST['id']."'");
           while($row = mysql_fetch_array($sql)) {
             $dep = $row['Department'];
             $id = $row['staff_id'];?>
             <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
           ...
           <br><div class="row">
             ...
             <div class="col">
               <b><span><label>Innovative Project Details</label></span></b>
             </div>
             <div class="col">
...
$sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.project_title,i.description,i.from_date,i.to_date,i.status from staff_academics a,staff_innovative i order by a.Department,a.Designation desc,i.from_date");
  $s =1;
  while($row = mysql_fetch_array($sql)){
    $name = $row['staff_id'];
  ...
  <tbody>
    ...
    <td style="width:200px; text-align:center;"><?php echo $desc;?></td>
    <td style="width:200px; text-align:center;"><?php echo $from;?></td>
    <td style="width:200px; text-align:center;"><?php echo $to;?></td>
    <td style="width:200px; text-align:center;"><?php echo $status;?></td>
    </tbody>
   <?php
    $s++;
        }
      }if(isset($_POST['submit1'])){
    $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.project_title,i.description,i.from_date,i.to_date,i.status from staff_academics a,staff_innovative i where a.staff_id=i.staff_id order by a.Department,a.Designation desc,i.from_date");
    $s =1;
    while($row = mysql_fetch_array($sql)){
      $name = $row['staff_id'];
      $staffname = $row['staff_name'];
    ...
    <tbody>
     ...
     <?php
      ...
      $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.project_title,i.description,i.from_date,i.to_date,i.status from staff_academics a,staff_innovative i where a.staff_id=i.staff_id and Department='".$_POST['dept']."' order by a.Designation desc,i.from_date");
      $s =1;
      while($row = mysql_fetch_array($sql)){
        $name = $row['staff_id'];
        $staffname = $row['staff_name'];
      ...
      $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.project_title,i.description,i.from_date,i.to_date,i.status from staff_academics a,staff_innovative i where a.staff_id=i.staff_id and ((i.from_date between '".$_POST['from']."' and '".$_POST['to']."') or (i.to_date between '".$_POST['from']."' and '".$_POST['to']."')) order by a.Department,a.Designation desc,i.from_date");
      $s =1;
      while($row = mysql_fetch_array($sql)){
        $name = $row['staff_id'];
        $staffname = $row['staff_name'];
