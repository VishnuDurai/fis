...
<div id="page">
	...
	<div style="color: #682D87;" class="news"><b><marquee behavior="alternate">Manage User</marquee></b></div>
<form action=" " onsubmit="return registerValidate(this)" enctype="multipart/form-data" method="post"><hr>
<div class="form-control"><br>
			<center><h3 style="color: #682D87;" class="form-group">Manage User </h3></center><hr>
		<table align="center">
...
<tr><td>Designation</td>
  <td><input type="text" class="form-control" name="Designation" class="form-control"
    value="<?php
    if(isset($_GET['staff_id'])){
      $id = $_GET['staff_id'];
      $type = $_GET['Designation'];
      echo $type;
    }
    ?>"
    style='background-color:white; font-weight:bold;' readonly>
		</td></tr>
<tr><td>Department</td>
...
<tr><td>&nbsp;</td><td><br>
  <input type="submit" class="btn btn-outline-success" onclick="window.location.reload(true)" style="cursor: pointer;" name="submit" value="Update"></td><td><br><center>
	<a href="exe_user.php"><button type="button" style="cursor: pointer;" class="btn btn-outline-primary">Back</button></center></td></tr>
</table><br>
</div>
</form>
<hr>
...
if(isset($_GET['staff_id'])){
  if(isset($_POST['submit'])){
    $id = $_GET['staff_id'];
    $type = $_POST['password'];
    $sql = mysql_query("update staff_user set password='$type' where staff_id='$id'");
if($sql){?>
	<script>
