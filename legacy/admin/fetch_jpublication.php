...
<html ng-app>
<head>
  <title>Report table Interaction</title>
...
<body bgcolor="tan">
  ...
  <center><h3 style="color: #682D87;">Journal Publication Report</h3></center>
     <hr>
<div class="form-inline">
<a href="jpublicationtable.php"><button type="button" style=" cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('header.php');?><center>
         <?php
         $sql = mysql_query("select distinct a.Department from staff_academics a,staff_publication i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
      ...
      <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
        ...
        ?><br><div class="row">
          ...
          <div class="col">
            <b><span><label>Journal Publication Report</label></span></b>
          </div>
          <div class="col">
        ...
        <?php
        $sql = mysql_query("select distinct type from staff_resource where to_date between '".$_POST['from']."' and '".$_POST['to']."'");
      while($row = mysql_fetch_array($sql))  {
        $depts = $row['type'];?>
       ...
       ?>
         ...
         <thead class="table-sm table-success">
           <tr>
             ...
             <th style="width:200px; text-align:center;">Name of the Journal</th>
             <th style="width:200px; text-align:center;">DOI</th>
             <th style="width:200px; text-align:center;">Month</th>
             <th style="width:200px; text-align:center;">Volume</th>
             <th style="width:200px; text-align:center;">PP</th>
             <th style="width:200px; text-align:center;">Scopus Indexed</th>
             <th style="width:200px; text-align:center;">Web of Science Indexed</th>
...
$sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.title,i.journel,i.doi,i.month_pub,i.volume_pub,i.pp,i.index_pub,i.web_of_science from staff_academics a,staff_publication i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."' order by a.Designation desc,i.month_pub");
  $s =1;
  while($row = mysql_fetch_array($sql)){
    $name = $row['staff_name'];
    $des = $row['Designation'];
...
$sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.title,i.journel,i.doi,i.month_pub,i.volume_pub,i.pp,i.index_pub,i.web_of_science from staff_academics a,staff_publication i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."' and month_pub between '".$_POST['from']."' and '".$_POST['to']."' order by a.Department,a.Designation desc,i.month_pub");
$s =1;
while($row = mysql_fetch_array($sql)){
  $name = $row['staff_name'];
  $des = $row['Designation'];
  ...
  $s++;
      ...
      $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.title,i.journel,i.doi,i.month_pub,i.volume_pub,i.pp,i.index_pub,i.web_of_science from staff_academics a,staff_publication i where a.staff_id = i.staff_id order by a.Department,a.Designation desc,i.month_pub");
      $s =1;
      while($row = mysql_fetch_array($sql)){
        $name = $row['staff_name'];
        $des = $row['Designation'];
