...
<body bgcolor="tan">
  ...
  <center><h3 style="color: #682D87;">Research Supervisor Details</h3></center>
     <hr>
     <div class="form-inline">
<a href="supervisortable.php"><button type="button" style="cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php include('header.php');?><center>
         <?php
         $sql = mysql_query("select distinct a.Department from staff_academics a,staff_supervisor i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
         while($row = mysql_fetch_array($sql)) {
           ...
           ?>
           <?php
           $sql = mysql_query("select distinct a.Department,i.staff_id from staff_academics a,staff_supervisor i where a.staff_id = i.staff_id and i.staff_id='".$_POST['staffid']."'");
           while($row = mysql_fetch_array($sql)) {
             $dep = $row['Department'];
             $id = $row['staff_id'];?>
             <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
             ...
             <br><div class="row">
               ...
               <div class="col">
                 <b><span><label>Research Supervisor Report</label></span></b>
               </div>
               <div class="col">
...
<table class="table table-sm table-bordered">
      ...
      if(isset($_POST['Getid'])){
        ...
        <tbody>
          ...
          <td style="width:200px; text-align:center;"><?php echo $supj;?></td>
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.res_sup_id,i.staff_id,i.staff_name,i.supj,i.university,i.internal,i.external,i.scholar from staff_academics a,staff_supervisor i where a.staff_id=i.staff_id and i.res_sup_id='".$_POST['resid']."'");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.res_sup_id,i.staff_id,i.staff_name,i.supj,i.university,i.internal,i.external,i.scholar from staff_academics a,staff_supervisor i where a.staff_id=i.staff_id and a.Department='".$_POST['dept']."' order by a.Designation desc");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.res_sup_id,i.staff_id,i.staff_name,i.supj,i.university,i.internal,i.external,i.scholar from staff_academics a,staff_supervisor i where a.staff_id=i.staff_id order by a.Designation desc");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
