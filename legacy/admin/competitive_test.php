...
<body bgcolor="tan"><br>
    <?php
$sql = "select * from staff_competitive order by staff_id";
...
<div style="color: #682D87;" class="news"><b><marquee behavior="alternate">Competitive Exam Details</marquee></b></div>
 <center><h3></h3></center>
  <hr>
  <div id="form-control">
...
<a href="competitive_test.php"><button type="button" style="margin-left: 30px; cursor: pointer;"
...
<table class="table table-bordered table-sm table-hover table-striped" id="myTable" style=" margin-top: 40px;">
  ...
  <?php
  $sql = "select a.Department,a.Designation,i.id,i.file,i.staff_id,i.staff_name,i.exam_name,i.level,i.score,i.date,i.date_of_certificate from staff_academics a,staff_competitive i where i.staff_id=a.staff_id";
  ...
  while($abc = mysql_fetch_array($result)){
      ...
      $patent = $abc['exam_name'];
      $institution = $abc['level'];
      $generation = $abc['score'];
      $data_of_exam = $abc['date_of_certificate'];
      $file = $abc['file'];
    ?>
  ...
  <tr>
    ...
    <td><?php echo $abc['level']; ?></td>
    <td><?php echo $abc['score']?></td>
    <td><?php echo $abc['date_of_certificate']?></td>
    <td><?php echo $abc['date']?></td>
    <td><a href="document/<?php echo $abc['file']; ?>" target="_blank">View</a></td>
    <td><?php echo "<a href='modify_competitive.php?id=$id&staff_id=$staff_id&staff_name=$staff_name&exam_name=$patent&level=$institution&score=$generation&date_of_certificate=$data_of_exam&file=$file' >Modify</a>" ?> </td>
  <td><?php echo "<a href='competitive_test.php?del=$abc[id]'>Delete</a>"; ?></td>
  </tr>
  </tbody>
  <?php
  }
...
{
	$id = $_GET['del'];
	$sql = "delete from staff_competitive where id='$id'";
...
if($result)
{
  ...
  alert('successfully uploaded');
        window.location.href='competitive_test.php?success';
        </script>
  <?php
}
else
{
  ...
  alert('error while uploading file');
        window.location.href='competitive_test.php?fail';
        </script>
  <?php
}
