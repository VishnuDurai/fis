...
<tr><td><button style="margin-left:46%;cursor:pointer;" class="btn btn-primary">Refresh</button></td></tr>
</div>
...
while($abc = mysql_fetch_array($result)){
  ...
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:100px; border:none; background-color:#FDF8E4;" value="'.$abc['from_date'].'" name="from'.$i.'" />';
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:100px; border:none; background-color:#FDF8E4;" value="'.$abc['to_date'].'" name="to'.$i.'" />';
  echo "</td>";
  echo "<td>";
...

  {
    if(isset($_POST['check'.$i.'']))
    {
      ...
      $update = "update staff_interaction set type='$type',title='$title',from_date='$from',to_date='$to',organizer='$org' where id='$id' ";
  ...
  if(isset($_POST['delete'.$i.'']))
  {
    if(isset($_POST['check'.$i.''])){
      ...
      $delete = "delete from staff_interaction where id='$id'";
