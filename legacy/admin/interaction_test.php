...
<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
 ...
 <input type="text" id="myInput" class="form-control" onkeyup="myFunction()" placeholder="Search Staff ID" style='background-color:white; font-weight:bold; width: 200px;'/><p>&nbsp;</p>&nbsp;&nbsp;&nbsp;&nbsp;
   ...
   <a href="interaction_test.php"><button type="button" style="margin-left: 30px; cursor: pointer;"
   class="btn btn-primary"> Refresh </button></a>
<form name="excel.php" method="post">
   <button type="button" name="create_excel" id="create_excel" style="margin-left: 30px; cursor: pointer;"
...
<table class="table table-sm table-bordered table-hover table-striped" id="myTable" style="margin-top: 40px;">
  ...
  <?php
  $sql = "select a.Department,a.Designation,i.id,i.file,i.staff_id,i.staff_name,i.type,i.title,i.from_date,i.to_date,i.organizer,i.date from staff_academics a,staff_interaction i where i.staff_id=a.staff_id order by i.from_date";
  $s=1;
  $result = mysql_query($sql);
  ...
  <tbody class="table-warning">
      <tr>
        ...
        <td><?php echo "<a href='modify_interaction.php?id=$id&staff_id=$staff_id&staff_name=$staff_name&type=$type&title=$title&from_date=$from&to_date=$to&organizer=$organizer&file=$file' >Modify</a>" ?> </td>
<td><?php echo "<a href='interaction_test.php?del=$row[id]'>Delete</a>"; ?></td>
      </tr>
    </tbody>
<?php
$s++;
...
{
	$id = $_GET['del'];
	$sql = "delete from staff_interaction where id='$id'";
	$result = mysql_query($sql) or die('Failed'.mysql_error());
if($result)
{?>
  <script>
  alert('successfully Deleted');
        window.location.href='interaction_test.php?success';
        </script>
  <?php
}
else
{
  ...
  alert('error while uploading file');
        window.location.href='interaction_test.php?fail';
        </script>
	<?php
}
}
