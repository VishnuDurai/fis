...
<body bgcolor="tan"><br>
    <?php
$sql = "select * from staff_member order by staff_id";
...
<table class="table table-sm table-bordered table-hover table-striped" id="myTable" style="margin-top: 40px;">
  ...
  <?php
  $sql = "select a.Department,a.Designation,i.id,i.staff_id,i.staff_name,i.membershipid,i.organization from staff_academics a,staff_member i where i.staff_id=a.staff_id order by i.organization";
  $s=1;
  $result = mysql_query($sql);
  ...
  <tbody class="table-warning">
      <tr>
        ...
        <td><?php echo "<a href='modify_professional.php?id=$id&staff_id=$staff_id&staff_name=$staff_name&type=$type&title=$title&from_date=$from&to_date=$to&organizer=$organizer&file=$file' >Modify</a>" ?> </td>
<td><?php echo "<a href='professional_test.php?del=$row[id]'>Delete</a>"; ?></td>
      </tr>
    </tbody>
<?php
$s++;
...
{
	$id = $_GET['del'];
	$sql = "delete from staff_member where id='$id'";
	$result = mysql_query($sql) or die('Failed'.mysql_error());
if($result)
{?>
  <script>
  alert('successfully Deleted');
        window.location.href='professional_test.php?success';
        </script>
  <?php
}
else
