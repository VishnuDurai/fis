...
<div style="color: #682D87;" class="news"><b><marquee behavior="alternate">STAFF AADHAR DETAILS</marquee></b></div>
 <center><h3></h3></center>
  <hr>
  <div id="form-control">
<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
 ...
 <input type="text" id="myInput" class="form-control" onkeyup="myFunction()" placeholder="Search Staff ID" style='background-color:white; font-weight:bold; width: 200px;'/><p>&nbsp;</p>&nbsp;&nbsp;&nbsp;&nbsp;
   ...
   <a href="view_pan.php"><button type="button" style="margin-left: 30px; cursor: pointer;"
...
<table class="table table-sm table-bordered table-hover table-striped" id="myTable" style="margin-top: 40px;">
  ...
  <?php
  $sql = "select a.Department,a.Designation,i.id,i.path1,i.staff_id,i.staff_name from staff_academics a,staff_aadhar i where i.staff_id=a.staff_id";
  $result = mysql_query($sql);
  ...
  <tbody class="table-warning">
      <tr>
        ...
        <td><?php echo $row['Department']; ?></td>
         ...
         <td><?php echo "<a href='view_pan.php?del=$row[id]'>Delete</a>"; ?></td>
      </tr>
    </tbody>
<?php
  }
...
{
	$id = $_GET['del'];
	$sql = "delete from staff_aadhar where id='$id'";
	$result = mysql_query($sql) or die('Failed'.mysql_error());
if($result)
{?>
  <script>
  alert('successfully Deleted');
        window.location.href='view_pan.php?success';
        </script>
  <?php
}
else
{
  ...
  alert('error while uploading file');
        window.location.href='view_pan.php?fail';
        </script>
  <?php
