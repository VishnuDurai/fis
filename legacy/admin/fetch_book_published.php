...
<a href="bookpublishedtable.php"><button type="button" style="margin-left: 30px; cursor: pointer;" class="btn btn-outline-danger btn-sm"> Back </button></a>
<div class="wrapper">
...
<input type="text" class="form-control col-sm-1" ng-model="ftext2" placeholder="Footer3">
     ...
     <div class="content">
       ...
       <?php
       $sql = mysql_query("select distinct a.Department from staff_academics a,staff_book_published i where a.staff_id = i.staff_id and a.Department='".$_POST['dept']."'");
       while($row = mysql_fetch_array($sql)) {
        ...
        ?>
         ...
         ?>
         <?php
         $sql = mysql_query("select distinct a.Department,i.staff_id from staff_academics a,staff_book_published i where a.staff_id = i.staff_id and i.staff_id='".$_POST['id']."'");
         while($row = mysql_fetch_array($sql)) {
           $dep = $row['Department'];
           $id = $row['staff_id'];?>
           <b><label class="offset-sm-1">Department of</label>&nbsp;<?php echo $dep;?></b>
        ...
        if(isset($_POST['Getid'])){
          $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.title,i.coauthor,i.publisher,i.edition,i.isbn from staff_academics a,staff_book_published i where a.staff_id = i.staff_id and i.staff_id='".$_POST['id']."' order by a.Designation desc");
          $s =1;
          while($row = mysql_fetch_array($sql)){
            $name = $row['staff_name'];
            $des = $row['Designation'];
          ...
          $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.title,i.coauthor,i.publisher,i.edition,i.isbn from staff_academics a,staff_book_published i where a.staff_id = i.staff_id order by a.Designation desc");
          $s =1;
          while($row = mysql_fetch_array($sql)){
            $name = $row['staff_name'];
            $des = $row['Designation'];
        ...
        $sql = mysql_query("select a.Department,a.Designation,i.staff_name,i.title,i.coauthor,i.publisher,i.edition,i.isbn from staff_academics a,staff_book_published i where a.staff_id=i.staff_id and a.Department='".$_POST['dept']."'");
        $s =1;
        while($row = mysql_fetch_array($sql)){
          $name = $row['staff_name'];
          $des = $row['Designation'];
