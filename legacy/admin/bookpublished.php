...
while($abc = mysql_fetch_array($result)){
  ...
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:150px; border:none; background-color:#FDF8E4;" value="'.$abc['coauthor'].'" name="coauthor'.$i.'" />';
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:100px; border:none; background-color:#FDF8E4;" value="'.$abc['publisher'].'" name="publisher'.$i.'" />';
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:100px; border:none; background-color:#FDF8E4;" value="'.$abc['edition'].'" name="edition'.$i.'" />';
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:150px; border:none; background-color:#FDF8E4;" value="'.$abc['isbn'].'" name="isbn'.$i.'" />';
  echo "</td>";
    echo "<td>";
...

  {
    if(isset($_POST['check'.$i.'']))
    {
      ...
      $update = "update staff_book_published set title='$type',coauthor='$title',publisher='$from',edition='$to',isbn='$org' where id='$id' ";

      $qry = mysql_query($update);
      if(!$qry){echo "UPDATED FAILED";}else
      {

        header("location:bookpublished.php");
  ...
  if(isset($_POST['delete'.$i.'']))
  {
    if(isset($_POST['check'.$i.''])){
      ...
      $delete = "delete from staff_book_published where id='$id'";
