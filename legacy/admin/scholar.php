...
<table class="table table-sm table-hover table-bordered table-striped" id="myTable" style="margin-top: 40px;">
  ...
  echo "</td>";
  echo "<td>";
  echo '<input type="number" style="width:60px; border:none; background-color:#FDF8E4;" value="'.$abc['res_id'].'" name="res_id'.$i.'" />';
  echo "</td>";
  echo "<td>";
  ...
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:150px; border:none; background-color:#FDF8E4;" value="'.$abc['sup_name'].'" name="sup_name'.$i.'" />';
  echo "</td>";
  echo "<td>";
  echo '<input type="text" style="width:100px; border:none; background-color:#FDF8E4;" value="'.$abc['desgination'].'" name="desgination'.$i.'" />';
  echo "</td>";
   echo "<td>";
  echo '<input type="text" style="width:170px; border:none; background-color:#FDF8E4;" value="'.$abc['organisation'].'" name="organisation'.$i.'" />';
  echo "</td>";
   echo "<td>";
...

  {
    if(isset($_POST['check'.$i.'']))
    {
      ...
      $from = $_POST['sup_name'.$i.''];
      $to=$_POST['desgination'.$i.''];
      $org = $_POST['organisation'.$i.''];
      $sta = $_POST['status'.$i.''];

      $update = "update staff_scholars set res_id='$type',university='$title',sup_name='$from',desgination='$to',organisation='$org',status='$sta' where id='$id' ";
 ...
 echo "<script type='text/javascript'>alert('Updated successfully !')</script>";
  ...
  if(isset($_POST['delete'.$i.'']))
  {
    if(isset($_POST['check'.$i.''])){
      ...
      $delete = "delete from staff_scholars where id='$id'";
