...
<body bgcolor="tan"><br>
    <?php
$sql = "select * from staff_certificate order by staff_id";
...
<div style="color: #682D87;" class="news"><b><marquee behavior="alternate">Online Certification Details</marquee></b></div>
 <center><h3></h3></center>
  <hr>
  <div id="form-control">
...
<a href="certificate_test.php"><button type="button" style="margin-left: 30px; cursor: pointer;"
...
<table class="table table-bordered table-sm table-hover table-striped" id="myTable" style=" margin-top: 40px;">
  ...
  <?php
  $sql = "select a.Department,a.Designation,i.id,i.file,i.staff_id,i.staff_name,i.course_name,i.mark,i.organisation,i.date,i.data_of_exam from staff_academics a,staff_certificate i where i.staff_id=a.staff_id";
  ...
  while($abc = mysql_fetch_array($result)){
      ...
      $patent = $abc['course_name'];
      $institution = $abc['mark'];
      $generation = $abc['organisation'];
      $data_of_exam = $abc['data_of_exam'];
      $file = $abc['file'];
    ?>
  ...
  <tr>
    ...
    <td><?php echo $abc['course_name']?></td>
    <td><?php echo $abc['mark']?></td>
    <td><?php echo $abc['organisation']?></td>
    <td><?php echo $abc['data_of_exam']?></td>
    <td><?php echo $abc['date']?></td>
    <td><a href="document/<?php echo $abc['file']; ?>" target="_blank">View</a></td>
    <td><?php echo "<a href='modify_certificate.php?id=$id&staff_id=$staff_id&staff_name=$staff_name&course_name=$patent&mark=$institution&organisation=$generation&file=$file' >Modify</a>" ?> </td>
  <td><?php echo "<a href='certificate_test.php?del=$abc[id]'>Delete</a>"; ?></td>
  </tr>
  </tbody>
  <?php
  }
...
{
	$id = $_GET['del'];
	$sql = "delete from staff_certificate where id='$id'";
